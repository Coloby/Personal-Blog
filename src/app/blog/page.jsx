      ...
      {posts?.map(post => {
        return (
          <article className="max-w-[300px] relative min-h-[366px]" key={post.slug} >
            ...
            >
              {/* 500 x 200 */}
              <div className="flex items-center justify-center w-full h-[180px] overflow-hidden rounded-xs">
                <img className=" object-cover w-full h-full rounded-xs" src={post.thumbnail ? "/assets/media/imgs/"+post.thumbnail : `https://picsum.photos/1000/1180?random=${post.index}`} alt="" />
              </div>
